...
%, in particular the \textbf{recommended} ones
% described in the section recommendations 
\note{We should then add a link here to wherever recommended Components etc. can be found once such a resource is available} are likely to have been tested in practice and have a good chance of being optimised for e.g.~facet mapping in the VLO. Therefore it is advised to always look for existing components that fully or partially meet your requirements (see \nameref{modelling-workflow}). Multiple existing components can be combined into new components (reused) if they fit, and optionally new components can be created using existing components as a template (this can be referred to as `recycling'), for example when additional elements within a component are required or different cardinalities are desired.
